...
def get_score_bd(lib):
    ...
    # return lib['sd']
    if one_element:
        return -lib['sd'] * 10000 + len(lib['books'])
    size = minimum(lib['bpd'] * (days_limit - lib['sd']), len(lib['books']))
    # print('minimum({}, {})'.format(lib['bpd'] * (days_limit - lib['sd']), len(lib['books'])))
    if size > 0:
        return (sum([s[i] for i in list(lib['books'].keys())[:size]]) / (lib['sd'] * (lib['sd'])))# + (lib['bpd'] * (days_limit - lib['sd']) - len(lib['books'])) / 20
    return 0
...
while days_limit > 0 and libs != []:
    ...
    for book in lib['book_placed']:
        for x in book_possesor[book]:
            del x['books'][book]
    sort_len = minimum(len(libs), 1500)
    libs = sorted(libs[:sort_len], key=lambda x : get_score_bd(x), reverse=True) + libs[sort_len:]
    days_limit -= lib['sd']
